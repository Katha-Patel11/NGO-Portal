...
<?php
  ...
  if(isset($_POST['submit']))
  {
    ...
    $q="select ngo_password from tbl_ngo_master where ngo_id='$ngo_id'";
    $result=mysqli_query($conn,$q);
    $row=mysqli_fetch_array($result);
    if($opass==$row['ngo_password'])
    {
      if($npass==$cpass)
      {
        if($opass==$npass)
        ...
        {
          $sql="update tbl_ngo_master set ngo_password='$npass' where ngo_id='$ngo_id'";
